...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/device/{device_id}", web::get().to(get_device))
    })
        .bind("127.0.0.1:8080")?
        .run()
        .await
...
async fn broadcast(
    msg: Path<String>,
    handler: Data<Mutex<HotPlugHandler>>,
) -> impl Responder {
    handler.lock().unwrap().send(&msg.into_inner());
...
async fn get_device(
    ...
    match handler.lock().unwrap().get_device(&device_id.into_inner()) {
        None => HttpResponse::NotFound().body("{}"),
        Some(device_info) => {
            HttpResponse::Ok().body(serde_json::to_string(&device_info).unwrap())
        }
    }
}
...
impl HotPlugHandler {
    ...
    fn get_device(&self, device_id: &String) -> Option<DeviceInfo> {
        for (id, device_info) in self.devices.iter() {
            if id.eq(device_id) {
